...
public class runing {
	...
	public static void main(String[] args) {
		...
		while(true) {												//第一层循环。---实现选择界面和主界面的来回切。
							...
							if(tmpx>=1&&tmpx<=tool.MAP_H &&tmpy>=1&&tmpy<=tool.MAP_W) {
								if(tool.MAP_TOP[tmpx][tmpy]==0) {
									...
									if(ga.Check()) {//是否踩到地雷？
										...
										URL url = getClass().getResource("../image/bad.png");
										Icon icon = new ImageIcon(url);
										ga.btn1.setIcon(icon);
//										ga.btn1.setIcon(new ImageIcon(Game.class.getResource("bad.png")));
										...
										URL url = getClass().getResource("../image/happy.png");
										Icon icon = new ImageIcon(url);
										ga.btn1.setIcon(icon);
//										ga.btn1.setIcon(new ImageIcon(Game.class.getResource("happy.png")));
							...
							if(tool.MAP_TOP[tmpx][tmpy]==0) {
								...
								URL url = getClass().getResource("../image/TopFlag.png");
								Icon icon = new ImageIcon(url);
								ga.lb2[tmpx][tmpy].setIcon(icon);
...
//									ga.btn1.setIcon(new ImageIcon(Game.class.getResource("happy.png")));
				...
				int time=1000;//因为无法实现在while中即实现1秒增加1.又要实现0.05秒检测一下是否退出。所以用了这个办法来解决。
